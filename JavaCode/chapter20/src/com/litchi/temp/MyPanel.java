...
public class MyPanel extends JPanel implements KeyListener, Runnable, Serializable {
    ...
    public MyPanel(int key) throws IOException, ClassNotFoundException {
        ...
        HashMap<Integer, Tank> hashMap = (HashMap<Integer, Tank>)o;
        System.out.println("hello Oldgame");
        Tank tank = hashMap.get(4);
        hero = new Hero(tank.getX(), tank.getY());//初始化自己坦克
        for (int i = 0; i < hashMap.size() - 1; i++) {
            Tank tank1 = hashMap.get(i);
            EnemyTank enemyTank = new EnemyTank(tank1.getX(), tank1.getY());
            new Thread(enemyTank).start();
            //给该enemyTank 加入一颗子弹
            Shot shot = new Shot(enemyTank.getX() + 20, enemyTank.getY() + 60, enemyTank.getDirect());
            //加入enemyTank的Vector 成员
...
//            }
    ...
    public void checkOverlap() {
        //思路
        //是确定每一辆的坦克，不能重叠，也就是我们需要多辆坦克互相对比
        //那就得拿到多量坦克的数据
        if (enemyTanks.size() == 1)
            return;
        for (int i = 0; i < enemyTanks.size(); i++) {
            EnemyTank srcTank = enemyTanks.get(i);
            for (int j = i + 1; j < enemyTanks.size(); j++) {
                EnemyTank destTank = enemyTanks.get(j);
                if (!(Math.abs(srcTank.getX() - destTank.getX()) <= 63 && Math.abs(srcTank.getY() - destTank.getY()) < 63))
                    continue;
                //操作他们的方向并且移动
                int temp = srcTank.getDirect();
                srcTank.setDirect(destTank.getDirect());
                destTank.setDirect(temp);
            }
        }
    }
