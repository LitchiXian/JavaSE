...
public class MyPanel extends JPanel implements KeyListener, Runnable {
    ...
    public MyPanel(boolean isNewGam) {
        //先判断记录的文件是否存在
        //如果存在，就正常执行，如果文件不存在，提示，只能开启新游戏
        String filePath = "src\\grade.txt";
        File file = new File(filePath);
        if (!isNewGam && !file.exists()) {
            System.out.println("没有旧游戏，只能开启新游戏(1.新游戏)");
            isNewGam = true;
        }
        ...
        if (isNewGam) {
            ...
            Vector<Node> tankNodes = Recorder.getTankNodes();
            //初始化敌人坦克
            for (int i = 0; i < tankNodes.size(); i++) {
                Node node = tankNodes.get(i);
                //创建一个敌人的坦克
    ...
    public void paintGrade(Graphics g) {
        g.setColor(Color.BLACK);
        g.drawString("您已累计击毁地方坦克", 1020, 30);
        g.drawString(Recorder.getGrade() + "", 1080, 80);
