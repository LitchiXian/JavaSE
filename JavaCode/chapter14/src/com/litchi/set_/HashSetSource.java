...
public class HashSetSource {
    public static void main(String[] args) {
        ...
        set.add("java");//到此位置，第1次add分析完毕
        set.add("php");//到此位置，第2次add分析完毕
        set.add("java");
        ...
        4. 执行 putVal
            final V putVal(int hash, K key, V value, boolean onlyIfAbsent,
                ...
                if ((p = tab[i = (n - 1) & hash]) == null)
                    ...
                    //并且满足下面两个条件之一:
                    //(1)准备加入的 key 和 p 指向的Node 结点key 是同一个对象
                    //(2) p 指向的Node 结点的 key 的equals() 和准备加入的 key比较后相同
                    //就不能加入
                    if (p.hash == hash &&
                    ...
                    //如果是一颗红黑树，就调用 putTreeVal ,来进行添加
                        ...
                        e = ((TreeNode<K,V>)p).putTreeVal(this, tab, hash, key, value);
                          ...
                          //   注意在吧元素添加到链表后，立即判断 该链表是否达到 8 个结点
                          //   如果达到，就调用 treeifyBin() 对当前这个链表进行树化(转成红黑树)
                          //   注意，在转成红黑树，要进行判断，判断条件
                          //   if (tab == null || (n = tab.length) < MIN_TREEIFY_CAPACITY【64】)
                          //            resize();
                          //   如果上面条件成立，先table扩容
                          //   只有上面条件不成立时，才进行转成红黑树
                          //(2)依次和该链表的每一个元素过程中，如果有相同情况，就break
